...
class Looper {
  constructor(x, y) {
    ...
    this.loopingSpeed = random(0.025, 0.1) * (random(1) < 0.5 ? 1 : -1);
    this.c = random(255);
    this.looping = false;
  ...
  step() {
    if (this.looping) {
      this.heading +=
        this.loopingSpeed + sin(this.sinOffset + frameCount * 0.1) * 0.05;
      ...
      if (
        this.maybeDoneLooping &&
        (this.heading < TAU * 0.05 || this.heading > TAU * 0.95)
      ) {
        this.looping = false;
        this.maybeDoneLooping = false;
        this.loopingSpeed = random(0.025, 0.1) * (random(1) < 0.5 ? 1 : -1);
      ...
      // not looping
      this.heading =
        randomGaussian() * 0.1 + sin(this.sinOffset + frameCount * 0.1) * 0.25;
      this.constrainHeading();

      if (this.x > width * 0.25 && random(1) < 0.01) {
        this.looping = true;
      }
    }
